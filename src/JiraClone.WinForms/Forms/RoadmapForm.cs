...
public sealed class RoadmapForm : UserControl
{
    ...
    private Control BuildBody()
    {
        ...
        split.HandleCreated += (_, _) => SplitContainerHelper.ConfigureSafeLayout(split, 220, 220, 420);
        split.SizeChanged += (_, _) => SplitContainerHelper.ConfigureSafeLayout(split, 220, 220, 420);
    ...
    private Control BuildTimelineAndDetail()
    {
        ...
        split.HandleCreated += (_, _) => SplitContainerHelper.ConfigureSafeLayout(split, 430, 0, 210);
        split.SizeChanged += (_, _) => SplitContainerHelper.ConfigureSafeLayout(split, 430, 0, 210);
        split.Panel1.BackColor = JiraTheme.BgSurface;
    ...
    private void OnEpicListDrawItem(object? sender, DrawItemEventArgs e)
    {
        ...
        TextRenderer.DrawText(e.Graphics, $"{epic.DoneCount}/{epic.ChildIssueCount} hoàn thành | {BuildDateRangeText(epic)}", JiraTheme.FontCaption, new Rectangle(e.Bounds.X + 70, e.Bounds.Y + 28, e.Bounds.Width - 86, 18), JiraTheme.TextSecondary, TextFormatFlags.EndEllipsis | TextFormatFlags.VerticalCenter);
    ...
    private static string BuildDateRangeText(RoadmapEpicDto epic) => epic.StartDate.HasValue && epic.DueDate.HasValue ? $"{epic.StartDate:dd MMM} - {epic.DueDate:dd MMM}" : epic.StartDate.HasValue ? $"B?t d?u {epic.StartDate:dd MMM}" : epic.DueDate.HasValue ? $"H?n {epic.DueDate:dd MMM}" : "Chua lên l?ch";
